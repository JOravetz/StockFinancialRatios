...
def read_prices_from_file(file_path, num_days):
    df = pd.read_csv(file_path, sep='\s+', header=None, names=['date', 'price'], usecols=['price'])
...
def calculate_sortino_ratio(returns, target_return=0):
    ...
    sortino_ratio = np.mean(returns - target_return) / downside_std if downside_std > 0 else 0
    return sortino_ratio

def print_ratios(symbol, days, cumulative_return, sharpe_ratio, sortino_ratio, information_ratio, treynor_ratio, calmar_ratio, omega_ratio, weighted_average, header=False):
    print(f"{symbol:<6s} {days:>5d} {cumulative_return * 100:>17.4f} {sharpe_ratio:>14.8f} {sortino_ratio:>14.8f} "
...
def process_stock(args, stock_file):
    ...
    symbol = Path(stock_file.name).stem  # extract the stock symbol from the file name

    # Read reference stock prices and input stock prices
    ref_prices = read_prices_from_file(reference, args.num_days)
    prices = pd.read_csv(stock_file, sep='\s+', header=None, names=['date', 'price'], usecols=['price']).tail(args.num_days)['price'].values
    ...
    sharpe_ratio = math.sqrt(len(prices)) * mean_daily_returns / std_daily_returns if std_daily_returns > 0 else 0
    ...
    information_ratio = mean_daily_returns / tracking_error if tracking_error > 0 else 0

    # Calculate the Treynor Ratio
    beta = np.cov(ref_daily_returns, daily_returns)[0,1] / np.var(ref_daily_returns)
    ...
    threshold_return = 0
    gains = np.sum(market_neutral_daily_returns[market_neutral_daily_returns > threshold_return])
    losses = np.sum(market_neutral_daily_returns[market_neutral_daily_returns <= threshold_return])
    ...
    return [symbol, args.num_days, cumulative_return, sharpe_ratio, sortino_ratio, information_ratio, treynor_ratio, calmar_ratio, omega_ratio, weighted_average]

    # Return the financial ratios as a list
    return [symbol, args.num_days, cumulative_return, sharpe_ratio, sortino_ratio, information_ratio, treynor_ratio, calmar_ratio, omega_ratio]

def print_header():
    print(f"{'Symbol':>6s} {'Days':>5s} {'Cumulative Return':>17s} {'Sharpe Ratio':>14s} {'Sortino Ratio':>14s} {'Information Ratio':>17s} "
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for symbol in stock_symbols:
                try:
                    ...
                    futures.append(executor.submit(process_stock, args, stock_file))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate financial ratios of stock prices using reference prices.")
    parser.add_argument("-r", "--reference", type=Path, default="./data/SPY.dat", help="Name of a file containing reference stock prices")
    parser.add_argument("-n", "--num_days", type=int, default=504, help="Number of days to use in the computations")
    parser.add_argument("-l", "--list", type=Path, help="File containing a list of stock symbols, one per line")
    parser.add_argument("-s", "--symbol", type=str, help="Stock symbol to process")
    parser.add_argument("-c", "--sort_column", type=int, default=9, help="Index of the column to sort by (0-based)")
